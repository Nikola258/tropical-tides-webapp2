...
<body>
    <div class="hero">
        ...
        <div class="hero-content">
            <div class="hero-title">Feedback & Beoordelingen</div>
        </div>
    </div>

    <div class="admin-dashboard-container" style="margin: 40px auto; max-width: 900px;">
        <h1>Beoordeel een Locatie</h1>
        <p>Deel je ervaring en help andere reizigers!</p>
        ...
        <div class="crud-item">
            <p style="margin-bottom: 15px;">Je plaatst een beoordeling als: <strong><?php echo htmlspecialchars($_SESSION['name']); ?></strong></p>
            <form action="include/feedback-action.php" method="post" class="crud-form" style="padding-top: 10px;">
                <label for="plaats">Kies een locatie:</label>
                ...
                <label for="user_review">Opmerking:</label>
                <textarea name="user_review" id="user_review" rows="2" style="width:100%; border-radius:6px; padding:8px;"></textarea>
                
                <input type="submit" name="feedback_submit" value="Plaats beoordeling">
            </form>
        </div>
        
        <h2 style="margin-top: 40px;">Alle Beoordelingen</h2>
        <?php if (empty($feedbacks)): ?>
            <p>Er zijn nog geen beoordelingen. Wees de eerste!</p>
        <?php else: ?>
            <?php foreach ($feedbacks as $fb): ?>
                <div class="crud-item" style="background: #f4f4f4; margin-top: 10px; border-left-color: #ffbf00;">
                    ...
                    <small style="display: block; color: #777; margin-top: 5px;">Geplaatst op: <?php echo date('d-m-Y', strtotime($fb['created_at'])); ?></small>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
