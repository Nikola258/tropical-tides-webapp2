...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Basic validation
    if (!$plaats || !$beschrijving || $prijs === false || $rating === false) {
        $_SESSION['message'] = "Please fill in all required fields with valid data.";
        $_SESSION['message_type'] = "danger";
    } else {
        try {
            $stmt = $conn->prepare(
                "INSERT INTO booking (plaats, beschrijving, prijs, img, rating) 
                 VALUES (:plaats, :beschrijving, :prijs, :img, :rating)"
            );
            ...
            $_SESSION['message'] = "Destination successfully added!";
            $_SESSION['message_type'] = "success";
        } catch (PDOException $e) {
            error_log("Destination creation error: " . $e->getMessage());
            $_SESSION['message'] = "A database error occurred. Could not add destination.";
            $_SESSION['message_type'] = "danger";
        }
    }
} else {
